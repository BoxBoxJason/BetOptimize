...
from ranking.MMR import processGames,START_DEVIATION
...
class FreeForAllWidget(TemplatePageWidget):
    ...
    def setDatabase(self,database_path):
        ...
        processGames(database_path,database['GAMES'],self.players_table,39.948612168502336,21.314329431412908,5675.089387551527,True)
    ...
    def __predictOutcome(self):
        ...
        if len(players_ids_list) > 1:
            ...
            players_dicts.sort(key= lambda x : x['SKILL'] - 3 * x['SKILL_DEVIATION'],reverse=True)
            self.clean()
            confidence_rate = 0
            for player_dict in players_dicts:
                self.__ranking_widget.addPlayerIdToList(player_dict['ID'])
                confidence_rate += (1 - player_dict['SKILL_DEVIATION'] / START_DEVIATION ) ** 5
            
            self.__prediction_confidence_rate_qlabel.setText(f"Prediction confidence rate: {((confidence_rate/len(players_dicts)) ** (1/5)) * 100}%")
